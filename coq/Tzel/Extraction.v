(** * Tzel.Extraction
    ...
    end-to-end commit the realizations are placeholders — both
    [Hash3] and [pack_adrs_chain] return a fixed zero felt — so the
    extracted code compiles and runs without depending on the OCaml
    [tzel] library. The placeholder makes [xmss_chain_step] degenerate
    (always zero) but exercises the Coq → OCaml extraction pipeline
    end-to-end.
    ...
    same input, and the differential driver (also planned for the next
    commit) will start exercising that equivalence.

    Note: extraction writes [tzel_wots.ml] / [tzel_wots.mli] to the
    directory [coqc] runs from. With our [_CoqProject], that's
    [coq/Tzel/]. The dune setup in [coq/Extracted/] copies the file
    into place for the OCaml driver.
*)
