(* coq/Extracted/main.ml — driver for the extracted Coq xmss_chain_step.
   ...
   With the placeholder hash + pack_adrs realizations in
   coq/Impl/Extraction.v, this driver always prints a zero felt — the
   point is exercising the build/extract/run pipeline end-to-end, not
   producing meaningful chain hashes. The next commit wires Hash3 and
   pack_adrs_chain to the bit-equivalent OCaml protocol port and adds
   a differential check against the Cairo [xmss_chain_step]. *)
